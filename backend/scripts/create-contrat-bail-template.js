/**
 * Script de génération du template DOCX du Contrat de Bail SARLU
 * Conforme au modèle de référence (2 pages, police Goudy Old Style 11pt)
 *
 * Usage: node backend/scripts/create-contrat-bail-template.js
 */
...
const FONT_SIZE = 22; // 11pt (comme le modèle)
const FONT_SIZE_TITLE = 56; // 28pt titre
...
const createContent = () => {
  ...
  children.push(para([
    ...
    text(', résidant à '),
    bold('{bailleur_adresse}'),
    text('   Téléphone :'),
    text('{bailleur_telephone}'),
  ]));
  children.push(para([text('Propriétaire, ci-après dénommé  « le bailleur »')]));
  children.push(centerPara([bold('D\'une part')], { spacing: { after: 40 } }));
  children.push(centerPara([bold('Et ')], { spacing: { after: 40 } }));

  children.push(para([
    text('La société dénommée «'),
    bold('{denomination_complete}'),
    text('».'),
  ]));
  children.push(para([
    text('Locataire ci-après dénommé  « le preneur » représenté par son gérant M. '),
    bold('{gerant_nom_complet}'),
  ]));
  ...
  children.push(para([
    ...
    text(' en vue de l\'exploitation de la SOCIETE   «'),
    bold('{denomination_complete}'),
    text('»'),
  ]));
    ...
    sections: [
      {
        properties: {
          page: {
            margin: {
              // Marges identiques au modèle de référence (en twips)
              top: 1134,
              bottom: 1134,
              left: 1418,
              right: 851,
...
const main = async () => {
  console.log('Création du template DOCX du Contrat de Bail SARLU...');

  const doc = createDocument();
  const buffer = await Packer.toBuffer(doc);

  const outputPath = path.join(__dirname, '../templates/contrat_bail_sarlu_template.docx');
